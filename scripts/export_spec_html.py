...
FILES = [ROOT / 'docs/APP_FRONTEND_SPEC.md', *sorted((ROOT / 'docs/spec-templates').glob('*.md'))]
CSS = '''
...
def export(path):
    ...
    for md in FILES:
        # Preserve other reference documents; export the three specification documents together.
        body = body.replace(f'{md.name}"', f'{md.with_suffix(".html").name}"')
