...
const NavButton = ({ icon: Icon, label, active, onClick, colorTheme }: NavButtonProps) => (
  <button
    onClick={onClick}
    className={`
      relative flex flex-col items-center justify-center h-full transition-all duration-300 group
  ...
  >
    ...
    {/* Label - Always White */}
    <span className="text-xs font-bold uppercase tracking-wider text-white drop-shadow-md transition-transform duration-300 whitespace-nowrap">
      {label}
    </span>
  </button>
);
