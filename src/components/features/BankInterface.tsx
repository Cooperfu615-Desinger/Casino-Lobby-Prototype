...
const BankInterface = ({ onClose, receiverId: initialReceiverId, initialTab }: BankInterfaceProps) => {
    ...
    const handleVaultDeposit = () => {
        ...
        if ((user?.balance.gold || 0) < val) {
            showToast('錢包金幣不足', 'error');
            return;
        }

        setLoading(true);
        setTimeout(() => {
            updateBalance({ gold: (user?.balance.gold || 0) - val });
            updateUser({ vault_gold: (user?.vault_gold || 0) + val });
            setLoading(false);
            showToast('成功存入保險箱', 'success');
            setVaultAmount('');
        }, 800);
    };
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            {/* Modal Container */}
            <div className="relative w-[90%] max-w-[1100px] min-h-[650px] max-h-[90vh] bg-[#1a0b2e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
