        ...
        <div className="relative flex h-full min-h-0 flex-col">
            <div className="mb-3 flex shrink-0 items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                    ...
                    {!merging && <LobbyModalButton onClick={() => setMerging(true)} disabled={cards.filter(card => canMergeRewardCard(card)).length < 2}><Merge size={15} />合併</LobbyModalButton>}
                </div>
            </div>
            <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                ...
                {cards.length === 0 ? <div className="flex min-h-64 flex-col items-center justify-center gap-4 text-center">
                    ...
                    {cards.map(card => {
                        const expired = isRewardCardExpired(card);
                        const eligible = canMergeRewardCard(card);
                        ...
                        return <article key={card.id} aria-label={card.title + (card.milestoneDay ? ` 第 ${card.milestoneDay} 天` : '')} className={`lobby-reward-card lobby-reward-card--silver min-w-0 rounded-2xl border p-3 ${selected.includes(card.id) ? 'border-white bg-white/15' : 'border-white/25'}`}>
                            ...
                            {merging ? <p className="mt-3 text-[10px] text-white/65">{eligible ? '可選取合併' : card.status === 'active' ? '使用中卡片請先退出合併模式並停用' : '此卡片不可合併'}</p> : <div className="mt-3 flex flex-wrap gap-2">
                                <LobbyModalButton disabled={!eligible} onClick={() => act(activateRewardCard(card.id), '獎勵卡已啟用')}><Play size={13} />啟用</LobbyModalButton>
            ...
            {merging && <div className="mt-3 shrink-0 rounded-xl bg-[#253479]/60 p-3" aria-label="合併預覽">
                <div className="mb-2 flex items-center justify-between"><span className="text-xs">已選取 {selected.length} 張</span><span className="text-[10px] text-white/65">至少選取兩張；合併後需重新啟用</span></div>
                {preview && <div className="mb-3 grid grid-cols-3 gap-2"><Metric label="目前餘額" value={preview.currentBalance} /><Metric label="總金額" value={preview.amount} /><Metric label="轉換上限" value={preview.conversionLimit} /><p className="col-span-3 text-[11px]">流水 {preview.totalTurnover.toLocaleString()} / {preview.turnoverTarget.toLocaleString()} · 有效期限 {preview.expiresAt}</p></div>}
                <div className="flex justify-end gap-2"><LobbyModalButton tone="secondary" onClick={reset}>取消</LobbyModalButton><LobbyModalButton disabled={!preview} onClick={() => { const result = mergeRewardCards(selected); if (result) { reset(); listRef.current?.scrollTo({ top: 0 }); showToast('已建立活動銀幣合併卡，請啟用後使用', 'success'); } else { setSelected([]); showToast('卡片狀態已變更，請重新選取', 'error'); } }}>確認合併</LobbyModalButton></div>
            </div>}
